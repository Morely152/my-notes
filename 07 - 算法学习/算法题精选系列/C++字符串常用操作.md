...
通常使用`getline(cin, str)`读取字符串。如果之前有使用`cin`,一定记得**使用`cin.ignore()`忽略掉换行符**，避免被getline捕获到：
...
需要注意的是，**`cin`和`getline()`有一些区别**：

> `cin >> str` 会在遇到第一个空白字符（空格、制表符、换行符）时停止读取，
> 而 `getline(cin, str)` 会读取整行直到遇到换行符。
> 例如，对于字符串 `"an apple"`，上述两种方法分别读取到 `"an"` 和 `"an apple"` 。
